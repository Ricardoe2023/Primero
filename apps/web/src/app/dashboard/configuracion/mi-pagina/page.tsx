  ...
  useEffect(() => {
    async function load() {
      ...
      if (!business && activeBizId) {
        const { data: fallback } = await supabase.from('businesses').select('id, name, slug, logo_url').eq('owner_id', user.id).order('created_at', { ascending: true }).limit(1).single()
        business = fallback
      }
      ...
      <div className="bg-white/[0.03] border border-white/[0.10] rounded-2xl px-5 py-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 min-w-0 px-3 py-2.5 rounded-xl bg-white/[0.05] border border-white/[0.08]">
            <p className="text-[12px] text-blue-400 truncate font-mono">{BASE_URL}/<span className="text-white">{slug}</span></p>
          </div>
          <button onClick={copyLink} className="shrink-0 px-3 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-black text-[12px] font-semibold transition-colors duration-150 flex items-center gap-1.5">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 bg-white/[0.05] border border-white/[0.10] rounded-xl px-3 py-2.5 text-[13px] text-white placeholder-white/20 focus:outline-none focus:border-blue-500/50 font-mono"
            />
          </div>
          <p className="text-[11px] text-white/20 mt-1 sm:hidden">gestai-app.vercel.app/<span className="text-blue-400/50">{slugInput || slug}</span></p>
          {slugInput && sanitize(slugInput) !== slugInput && (
            <p className="text-[11px] text-white/30 mt-1">Quedará como: <span className="text-blue-400/70 font-mono">{sanitize(slugInput)}</span></p>
          )}
        </div>
