...
export default function BusinessBot({ businessId, businessName, selectedStaffName, pendingMessage, onPendingConsumed }: {
  ...
  const WELCOME: Message = {
    id: 0,
    from: 'bot',
    text: `¡Hola! 👋 Soy el asistente de **${businessName}**.\n¿En qué te puedo ayudar? Puedo orientarte sobre servicios, precios o agendar tu hora.`,
  }
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const res = await fetch('/api/assistant', {
        ...
        body: JSON.stringify({ messages: newHistory, businessId, staffName: selectedStaffName }),
      })
            ...
            <div className="rounded-[calc(1.75rem-5px)] bg-[#0e0d0c] flex flex-col overflow-hidden h-full">
              ...
              <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.06] shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-white leading-none truncate">{businessName}</p>
                  <p className="text-[11px] text-white/35 mt-0.5">Asistente IA · En línea</p>
                </div>
                <button
                  onClick={() => setOpen(false)}
                  className="w-6 h-6 rounded-full bg-white/[0.06] hover:bg-white/[0.12] flex items-center justify-center transition-colors duration-150 shrink-0"
                >
